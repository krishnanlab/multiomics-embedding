...
def get_scores(results: dict, best_idx: int) -> dict:
    ...
    for metric in ["f1", "balanced_accuracy", "accuracy"]:
        scores[metric] = {
            i: results[f"split{i}_test_{metric}"][best_idx] for i in range(5)
        }
    return scores
...
def train_full_model(
    ...
    save_model_weights(model=log_reg, model_type=model_type, out_root=out_root, tag=tag)
    save_model(model=log_reg, model_type=model_type, out_root=out_root, tag=tag)
    generate_feature_predictions(
        model=log_reg, model_type=model_type, out_root=out_root, tag=tag, p=p, q=q, g=g
    )
...
def main(p: float, q: float, g: int, out_dir: str, tag: str) -> None:
    ...
    # load embedding
    time_emb, diet_emb = load_embedding(
        p, q, g, time_indices["nodes"], diet_indices["nodes"]
    )
    ...
    train_full_model(
        best_params=time_search.best_params_,
        emb=time_emb,
        labels=time_labels,
        model_type="time",
        out_root=out_dir,
    ...
    train_full_model(
        best_params=diet_search.best_params_,
        emb=diet_emb,
        labels=diet_labels,
        model_type="diet",
        out_root=out_dir,
