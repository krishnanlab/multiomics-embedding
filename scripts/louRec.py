...
parser = argparse.ArgumentParser(
    description="Run Louvain Clustering multiple times and extract connected components."
)
parser.add_argument(
    "--NumLouvain",
    type=int,
    default=100,
    help="Number of times to run Louvain clustering.",
)
parser.add_argument(
    "--Cooccur_percent",
    type=float,
    default=0.9,
    help="Percentage threshold for co-occurrence.",
)
parser.add_argument(
    "--Resolution",
    type=float,
...
for _ in range(k):
    ...
    for community in set(partition.values()):
        community_nodes = [
            node for node, comm in partition.items() if comm == community
        ]
